...
BranchPredictor* BranchPredictor::create(core_id_t core_id)
{
   try
   {
      ...
      else if (type == "pentium_m")
      {
         ...
         return new GsharePredictor("branch_predictor", core_id, size);
      }
      else if (type == "bimode") {
         float phtRatio = cfg->getFloatArray("perf_model/branch_predictor/phtRatio", core_id);
         UInt32 bhrSize = cfg->getIntArray("perf_model/branch_predictor/bhrSize", core_id);
         return new BiModePredictor("branch_predictor", core_id, size, phtRatio, bhrSize);
      }
      else if (type == "perceptron") {
         UInt32 perceptrons = cfg->getIntArray("perf_model/branch_predictor/perceptrons", core_id);
         UInt32 bhrSize = cfg->getIntArray("perf_model/branch_predictor/bhrSize", core_id);
         UInt32 threshold = cfg->getIntArray("perf_model/branch_predictor/threshold", core_id);
         return new PerceptronPredictor("branch_predictor", core_id, size, perceptrons, bhrSize, threshold);
      }
      else
      {
